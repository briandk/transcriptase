...
describe("Padding timestamps out with hours", (): void => {
  it("should pad short timestamps with two zeros for the minutes", (): void => {
    const doubleZeroMinutesSeconds = "[00:42]"
    const expectedTimecode = "00:00:42"

    expect(padToHoursMinutesSeconds(doubleZeroMinutesSeconds)).toBe(
      expectedTimecode,
    )
  })
  it("should pad short single-minute timestamps with no leading zero", (): void => {
    const noLeadingZeroMinutesSeconds = "[2:43]"
    const expectedTimecode = "00:2:43"

    expect(padToHoursMinutesSeconds(noLeadingZeroMinutesSeconds)).toBe(
      expectedTimecode,
    )
  })
  it("should pad double-digit minutes with no leading hours", (): void => {
  ...
  it("should leave single-digit hours unpadded", (): void => {
  ...
  it("should handle fractions of a second", (): void => {
    ...
    expect(padToHoursMinutesSeconds(minutesSecondsDecimal)).toBe(
      expectedTimecode2,
    )
